...
#    kafkaClient=pykafka.KafkaClient(hosts="wn0-first.cppnfcf2hhguzcmuway5ojjrwb.bx.internal.cloudapp.net:9092")
    ...
    crimesDF=spark.readStream.format("csv") \
                                ...
                                .schema(schema) \
                                .load("/mnt/c/Users/jvisvanathan/PycharmProjects/PycharmExample/SparkStreaming/datasets/dropLocation")
    ...
    convictionsPerCategory=spark.sql("select /*+ REPARTITION(8) */ major_category, SUM(value) as Conviction_Count from LondonCrimesTbl where year='2016' group by major_category ")

#    query=convictionsPerCategory.selectExpr("CAST(major_category AS STRING) as key", \
#                                            "to_json(struct(*)) as value") \

#   For Kafka Outputs all modes are Supported
#   Append mode is supported only when no Aggregation is performed
